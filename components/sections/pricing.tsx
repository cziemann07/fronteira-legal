    ...
    <section id="servicos" className="border-t border-border bg-background">
      <div className="mx-auto max-w-6xl px-4 py-16 md:px-6 md:py-24">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <Reveal as="p" className="mt-4 text-pretty text-base leading-relaxed text-muted-foreground">
            {t.pricing.subtitle}
          </Reveal>
        </div>
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:mt-14 md:grid-cols-3">
          {t.pricing.plans.map((plan, i) => (
            <Reveal
              key={plan.name}
              delay={i * 90}
              className="flex flex-col gap-5 rounded-xl border border-border bg-card p-6 md:p-7"
            >
              <div className="flex flex-col gap-2">
                <h3 className="text-lg font-semibold leading-snug text-foreground">{plan.name}</h3>
                <div className="flex items-baseline gap-1.5">
                  <span className="text-sm font-medium text-muted-foreground">{t.pricing.currency}</span>
